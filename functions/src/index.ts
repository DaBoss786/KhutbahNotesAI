...
const YOUTUBE_CAPTIONS_MAX_CHARS = 450000;
const YOUTUBE_CAPTIONS_FETCH_ATTEMPTS = 3;
const YOUTUBE_CAPTIONS_RETRY_DELAY_MS = 450;
...
export const onAudioUpload = onObjectFinalized(
  ...
  async (event) => {
    ...
    try {
      ...
      const primaryTranscriptionModel = "gpt-4o-transcribe";
      const fallbackTranscriptionModel = "whisper-1";
      type ChunkResult = {
        text: string;
        model: string;
      ...
      for (let i = 0; i < chunkPaths.length; i++) {
        ...
        try {
          ...
          logger.warn("Transcription chunk fallback triggered.", {
            lectureId,
            chunkIndex,
            totalChunks,
        ...
        if (!chunkText && modelUsed !== fallbackTranscriptionModel) {
          fallbackTriggered = true;
...
/**
 * Pipeline error that tracks the stage where processing failed.
 */
class MasjidPipelineError extends Error {
  stage: string;

  /**
   * Creates a masjid pipeline error.
...
/**
 * Returns a short one-line transcript preview.
 * @param {string} text Full transcript text.
 * @return {string} Preview text capped for list display.
 */
...
/**
 * Fetches captions transcript for a YouTube video if available.
 * @param {string} youtubeVideoId YouTube video ID.
 * @return {Promise<object|null>} Transcript text and metadata, or null.
 */
...
/**
 * Fetches a URL as text with retries.
 * @param {string} url URL to fetch.
 * @param {string} stage Pipeline stage for surfaced errors.
 * @return {Promise<string>} Response body as text.
 */
async function fetchTextWithRetries(
  ...
  for (let attempt = 1; attempt <= YOUTUBE_CAPTIONS_FETCH_ATTEMPTS; attempt++) {
    try {
      ...
      lastError = error;
      if (attempt < YOUTUBE_CAPTIONS_FETCH_ATTEMPTS) {
        const backoff = attempt * YOUTUBE_CAPTIONS_RETRY_DELAY_MS;
        await delay(backoff);
      }
    }
  }
  ...
  throw new MasjidPipelineError(
    stage,
    `Failed to fetch YouTube captions data (${message}).`
  );
}

/**
 * Extracts a JSON object that appears after a marker string.
...
/**
 * Extracts YouTube player response from watch page HTML.
...
/**
 * Converts unknown track data into a normalized caption track object.
 * @param {unknown} rawTrack Raw caption track payload.
 * @return {YouTubeCaptionTrack | null} Normalized track or null.
 */
function normalizeCaptionTrack(rawTrack: unknown): YouTubeCaptionTrack | null {
  if (!isPlainRecord(rawTrack)) {
    return null;
  }
  const baseUrl = rawTrack.baseUrl;
  const languageCode = rawTrack.languageCode;
  const kind = rawTrack.kind;
  ...
  const isAutoGenerated =
    kind === "asr" ||
    (typeof vssId === "string" && vssId.startsWith("a."));
  return {baseUrl, languageCode, isAutoGenerated};
}

/**
 * Reads caption tracks from a YouTube player response object.
 * @param {Record<string, unknown>} playerResponse Parsed player response.
 * @return {YouTubeCaptionTrack[]} Available caption tracks.
 */
function extractCaptionTracks(
  playerResponse: Record<string, unknown>
): YouTubeCaptionTrack[] {
  const captions = playerResponse.captions;
  if (!isPlainRecord(captions)) {
    return [];
  }
  const renderer = captions.playerCaptionsTracklistRenderer;
  if (!isPlainRecord(renderer)) {
    return [];
  }
...
/**
 * Chooses the best available caption track.
 * @param {YouTubeCaptionTrack[]} tracks All caption tracks.
 * @return {YouTubeCaptionTrack | null} Selected track or null.
 */
function chooseCaptionTrack(
  ...
  const preferredEnglish = pool.find((track) =>
    track.languageCode === "en" || track.languageCode.startsWith("en-")
  );
  return preferredEnglish ?? pool[0];
}

/**
 * Decodes a subset of HTML entities used in caption payloads.
 * @param {string} input Raw text potentially containing entities.
...
/**
 * Normalizes a caption fragment into plain one-line text.
 * @param {string} value Raw caption fragment.
 * @return {string} Normalized text fragment.
 */
function normalizeCaptionFragment(value: string): string {
...
/**
 * Joins caption lines while removing immediate duplicates.
...
/**
 * Parses caption text from YouTube json3 caption payload.
...
function parseJson3Captions(payload: string): string {
  try {
    ...
    for (const event of events) {
      if (!isPlainRecord(event) || !Array.isArray(event.segs)) {
        continue;
      }
      const text = event.segs
        .map((seg) => {
          if (!isPlainRecord(seg)) {
            return "";
          }
          return typeof seg.utf8 === "string" ? seg.utf8 : "";
        })
        .join("");
...
/**
 * Parses caption text from XML caption payload.
 * @param {string} payload Caption payload.
 * @return {string} Parsed transcript text.
 */
function parseXmlCaptions(payload: string): string {
  ...
  let match: RegExpExecArray | null = textRegex.exec(payload);
  while (match) {
...
/**
 * Caps transcript length to keep summarization costs predictable.
 * @param {string} text Transcript text.
 * @param {string} youtubeVideoId YouTube video ID for logging context.
 * @return {string} Possibly truncated transcript text.
 */
function capCaptionTranscriptLength(
  text: string,
  youtubeVideoId: string
): string {
  if (text.length <= YOUTUBE_CAPTIONS_MAX_CHARS) {
    return text;
  }
  logger.warn("Caption transcript exceeded max size; truncating.", {
    youtubeVideoId,
    charCount: text.length,
    maxChars: YOUTUBE_CAPTIONS_MAX_CHARS,
  });
  return text.slice(0, YOUTUBE_CAPTIONS_MAX_CHARS).trim();
}

/**
 * Builds a caption URL from YouTube's base caption URL.
 * @param {string} baseUrl Base URL from YouTube caption track.
 * @return {string} URL with preferred format parameters.
 */
function buildCaptionUrl(baseUrl: string): string {
...
/**
 * Parses XML attributes from a single tag fragment.
 * @param {string} rawTag Raw tag attributes content.
 * @return {Record<string, string>} Attribute name/value map.
 */
function parseXmlAttributes(rawTag: string): Record<string, string> {
  const attributes: Record<string, string> = {};
  const attributeRegex = /([A-Za-z_:-]+)="([^"]*)"/g;
  let match: RegExpExecArray | null = attributeRegex.exec(rawTag);
  while (match) {
    const key = match[1];
...
/**
 * Parses YouTube timedtext track list XML into normalized caption tracks.
 * @param {string} payload Timedtext track list XML payload.
 * @param {string} youtubeVideoId YouTube video ID.
 * @return {YouTubeCaptionTrack[]} Normalized track list.
 */
function parseTimedTextTrackList(
  ...
  let match: RegExpExecArray | null = trackRegex.exec(payload);
  while (match) {
    const attributes = parseXmlAttributes(match[1] ?? "");
    const languageCode = attributes.lang_code?.trim();
    if (!languageCode) {
      match = trackRegex.exec(payload);
      continue;
    }
    ...
    if (kind) {
      timedTextUrl.searchParams.set("kind", kind);
    }

    const name = attributes.name?.trim();
    if (name) {
      timedTextUrl.searchParams.set("name", name);
    }

    const baseUrl = timedTextUrl.toString();
...
/**
 * Fetches captions using YouTube's timedtext endpoint.
 * @param {string} youtubeVideoId YouTube video ID.
 * @return {Promise<YouTubeCaptionResult|null>} Transcript text and metadata.
 */
async function fetchYouTubeCaptionsViaTimedText(
  youtubeVideoId: string
): Promise<YouTubeCaptionResult | null> {
  const encodedVideoId = encodeURIComponent(youtubeVideoId);
  const trackListUrls = [
    `https://video.google.com/timedtext?type=list&v=${encodedVideoId}`,
    `https://www.youtube.com/api/timedtext?type=list&v=${encodedVideoId}`,
  ];
  let tracks: YouTubeCaptionTrack[] = [];
  for (const trackListUrl of trackListUrls) {
    try {
      const trackListPayload = await fetchTextWithRetries(
        trackListUrl,
        "fetch_timedtext_tracklist"
      );
      const parsedTracks = parseTimedTextTrackList(
        trackListPayload,
        youtubeVideoId
      );
      ...
      logger.warn("Timedtext track list returned no tracks.", {
        youtubeVideoId,
        endpoint: trackListUrl,
        payloadLength: trackListPayload.length,
      });
    } catch (error) {
      logger.warn("Timedtext track list fetch failed.", {
        youtubeVideoId,
        endpoint: trackListUrl,
  ...
  logger.info("Selected YouTube timedtext caption track.", {
    youtubeVideoId,
    languageCode: selectedTrack.languageCode,
    isAutoGenerated: selectedTrack.isAutoGenerated,
  });

  const timedTextJsonUrl = buildCaptionUrl(selectedTrack.baseUrl);
  let captionsPayload = await fetchTextWithRetries(
    timedTextJsonUrl,
    "fetch_timedtext_captions"
  );
  let transcriptText = parseJson3Captions(captionsPayload);
  if (!transcriptText) {
    transcriptText = parseXmlCaptions(captionsPayload);
  }
  if (!transcriptText) {
    const timedTextXmlUrl = new URL(selectedTrack.baseUrl);
    timedTextXmlUrl.searchParams.delete("fmt");
    captionsPayload = await fetchTextWithRetries(
      timedTextXmlUrl.toString(),
      "fetch_timedtext_captions"
    );
    transcriptText = parseXmlCaptions(captionsPayload);
  }

  transcriptText = capCaptionTranscriptLength(transcriptText, youtubeVideoId);
  if (!transcriptText) {
    return null;
  }

  logger.info("Fetched YouTube captions via timedtext endpoint.", {
    youtubeVideoId,
    languageCode: selectedTrack.languageCode,
    isAutoGenerated: selectedTrack.isAutoGenerated,
  ...
  const source = selectedTrack.isAutoGenerated ?
    "youtube_timedtext_auto" :
    "youtube_timedtext";
  return {
    text: transcriptText,
    source,
...
/**
 * Converts transcript segments into normalized text.
 * @param {Array<{text: string}>} segments Transcript segments.
 * @return {string} Normalized transcript text.
 */
function normalizeTranscriptApiSegments(
  segments: Array<{text: string}>
): string {
  const normalized = segments
    .map((segment) => normalizeCaptionFragment(segment.text))
    .filter((segmentText) => segmentText.length > 0);
  return dedupeAndJoinCaptionLines(normalized);
}
 ...
 * @return {Promise<YouTubeCaptionResult|null>} Transcript text and metadata.
 */
async function fetchYouTubeCaptionsViaTranscriptApi(
  youtubeVideoId: string
): Promise<YouTubeCaptionResult | null> {
  try {
    const segments = await YoutubeTranscriptApi.fetchTranscript(youtubeVideoId);
    if (!segments.length) {
      return null;
    }
    const transcriptText = capCaptionTranscriptLength(
      normalizeTranscriptApiSegments(segments),
      youtubeVideoId
    );
    if (!transcriptText) {
      return null;
    }
    const languageCode = typeof segments[0]?.lang === "string" ?
      segments[0].lang :
      "unknown";
    logger.info("Fetched YouTube captions via transcript API.", {
      youtubeVideoId,
      languageCode,
      charCount: transcriptText.length,
    });
    ...
    logger.warn("youtube-transcript fetch failed; falling back to parser.", {
      youtubeVideoId,
      error: error instanceof Error ? error.message : "Unknown",
    });
    return null;
...
/**
 * Fetches transcript using the youtube-transcript-plus npm package.
 * @param {string} youtubeVideoId YouTube video ID.
 * @return {Promise<YouTubeCaptionResult|null>} Transcript text and metadata.
 */
async function fetchYouTubeCaptionsViaTranscriptPlus(
  youtubeVideoId: string
): Promise<YouTubeCaptionResult | null> {
  try {
    const transcriptResponse = await fetchTranscriptPlus(youtubeVideoId);
    ...
    const languageCode = typeof segments[0]?.lang === "string" ?
      segments[0].lang :
      "unknown";
    logger.info("Fetched YouTube captions via transcript-plus.", {
      youtubeVideoId,
      languageCode,
      charCount: transcriptText.length,
    });
    ...
    logger.warn("youtube-transcript-plus fetch failed; falling back.", {
      youtubeVideoId,
      error: error instanceof Error ? error.message : "Unknown",
    });
    return null;
...
/**
 * Fetches captions by parsing YouTube watch page and caption tracks.
 * @param {string} youtubeVideoId YouTube video ID.
 * @return {Promise<YouTubeCaptionResult|null>} Transcript text and metadata.
 */
async function fetchYouTubeCaptionsViaPageParser(
  youtubeVideoId: string
): Promise<YouTubeCaptionResult | null> {
  const watchUrl = `https://www.youtube.com/watch?v=${youtubeVideoId}`;
  const watchHtml = await fetchTextWithRetries(watchUrl, "fetch_caption_page");
  const playerResponse = extractPlayerResponseFromWatchHtml(watchHtml);
  if (!playerResponse) {
    throw new MasjidPipelineError(
      "fetch_caption_page",
      "Could not parse YouTube player response for captions."
    );
  }
  ...
  logger.info("Selected YouTube caption track.", {
    youtubeVideoId,
    languageCode: selectedTrack.languageCode,
    isAutoGenerated: selectedTrack.isAutoGenerated,
  });

  const captionsUrl = buildCaptionUrl(selectedTrack.baseUrl);
  const captionsPayload = await fetchTextWithRetries(
    captionsUrl,
    "fetch_captions"
  );
  ...
  logger.info("Fetched YouTube captions transcript.", {
    youtubeVideoId,
    languageCode: selectedTrack.languageCode,
    isAutoGenerated: selectedTrack.isAutoGenerated,
    charCount: transcriptText.length,
 ...
 * @return {Promise<YouTubeCaptionResult|null>} Transcript text and metadata.
 */
async function fetchYouTubeCaptionsTranscript(
  youtubeVideoId: string
): Promise<YouTubeCaptionResult | null> {
  const transcriptApiResult =
    await fetchYouTubeCaptionsViaTranscriptApi(youtubeVideoId);
  if (transcriptApiResult) {
    return transcriptApiResult;
  }

  const transcriptPlusResult =
    await fetchYouTubeCaptionsViaTranscriptPlus(youtubeVideoId);
  if (transcriptPlusResult) {
    return transcriptPlusResult;
  }

  try {
    const timedTextResult =
      await fetchYouTubeCaptionsViaTimedText(youtubeVideoId);
    if (timedTextResult) {
      return timedTextResult;
    }
  } catch (error) {
    logger.warn("Timedtext caption fetch failed; falling back to parser.", {
      youtubeVideoId,
      error: error instanceof Error ? error.message : "Unknown",
    });
  }
  ...
  try {
    return await fetchYouTubeCaptionsViaPageParser(youtubeVideoId);
  } catch (error) {
    if (
      error instanceof MasjidPipelineError &&
      error.stage === "fetch_caption_page" &&
      error.message === "Could not parse YouTube player response for captions."
    ) {
      logger.warn(
        "Page parser could not extract YouTube player response; " +
          "no captions found.",
        {youtubeVideoId}
      );
      return null;
...
export const processMasjidKhutbahQueue = onDocumentWritten(
  {
    document: "masjids/{masjidId}/khutbahs/{khutbahId}",
    region: "us-central1",
    secrets: [openaiKey],
    timeoutSeconds: 540,
    memory: "1GiB",
  ...
  async (event) => {
    ...
    const acquired = await db.runTransaction(async (tx) => {
      ...
      if (!snap.exists || current?.status !== "queued") {
        return false;
      }
    ...
    try {
      const processingSnap = await docRef.get();
      const processingData =
        processingSnap.data() as Record<string, unknown> | undefined;
      if (!processingData) {
        return;
      }

      const youtubeVideoId = typeof processingData.youtubeVideoId === "string" ?
        processingData.youtubeVideoId.trim() :
        "";
      ...
      if (typeof processingData.manualTranscript === "string") {
        manualTranscript = processingData.manualTranscript.trim();
      }
      let transcriptResult: YouTubeCaptionResult | null = null;
      if (manualTranscript) {
        const cappedManualTranscript = capCaptionTranscriptLength(
          manualTranscript,
          youtubeVideoId
        );
        if (!cappedManualTranscript) {
          throw new MasjidPipelineError(
            "validate_transcript",
            "Manual transcript is empty."
          );
        }
        transcriptResult = {
          text: cappedManualTranscript,
          source: "admin_manual",
          languageCode: "manual",
          isAutoGenerated: false,
        };
        logger.info("Using manual transcript provided by admin.", {
          masjidId: event.params.masjidId,
          khutbahId: event.params.khutbahId,
          youtubeVideoId,
          charCount: cappedManualTranscript.length,
        });
      } else {
        transcriptResult = await fetchYouTubeCaptionsTranscript(youtubeVideoId);
        if (!transcriptResult?.text?.trim()) {
          throw new MasjidPipelineError(
            "fetch_captions",
            "Captions unavailable for this YouTube video."
          );
        }
      }
      ...
      const perChunkTokenLimit =
        chunks.length === 1 ? MAX_SUMMARY_OUTPUT_TOKENS : CHUNK_OUTPUT_TOKENS;
      const chunkSummaries: SummaryShape[] = [];
      for (let i = 0; i < chunks.length; i++) {
        const chunkSummary = await summarizeChunk(
          openai,
      ...
      const transcriptRef = docRef.collection("artifacts").doc("transcript");
      ...
      await docRef.set({
        title:
          typeof processingData.title === "string" &&
            processingData.title.trim().length > 0 ?
            processingData.title.trim() :
            "Khutbah Summary",
        mainTheme: safeSummary.mainTheme,
        keyPoints: safeSummary.keyPoints,
        explicitAyatOrHadith: safeSummary.explicitAyatOrHadith,
        weeklyActions: safeSummary.weeklyActions,
        transcriptPreview: buildTranscriptPreview(transcriptText),
        transcriptRefPath: transcriptRef.path,
        transcriptCharCount: transcriptText.length,
        transcriptSource: transcriptResult.source,
        transcriptLanguageCode: transcriptResult.languageCode,
        transcriptIsAutoGenerated: transcriptResult.isAutoGenerated,
        status: "ready",
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      ...
      const masjidRef = docRef.parent.parent;
      if (masjidRef) {
        await masjidRef.set({
      ...
      const stage =
        err instanceof MasjidPipelineError ? err.stage : "summarize";
      const errorMessage =
        err instanceof Error ? err.message : "Khutbah processing failed.";
      logger.error("Masjid khutbah processing failed.", {
        masjidId: event.params.masjidId,
        khutbahId: event.params.khutbahId,
        stage,
        errorMessage,
...
/**
 * Parses a comma-separated UID secret into a lookup set.
 * @param {string | undefined} raw Secret value from `ADMIN_UIDS`.
 * @return {Set<string>} Set of admin UIDs.
 */
function parseAdminUidSet(raw: string | undefined): Set<string> {
  if (!raw) {
    return new Set();
  }
  const values = raw
...
/**
 * Returns whether a UID is allowed to use masjid admin endpoints.
 * @param {string} uid Firebase Auth UID.
 * @return {boolean} True when UID exists in configured admin set.
 */
function isMasjidAdmin(uid: string): boolean {
  const adminUidSet = parseAdminUidSet(masjidAdminUids.value());
  return adminUidSet.has(uid);
}

/**
 * Resolves UID from `Authorization: Bearer <token>` header.
 * @param {Request} req Express request.
 * @return {Promise<string | null>} UID or null when auth is missing/invalid.
 */
async function getUidFromBearerAuth(req: Request): Promise<string | null> {
  const authHeader = req.headers.authorization ?? "";
  const match = authHeader.match(/^Bearer (.+)$/);
  if (!match) {
    return null;
...
/**
 * Validates YouTube's canonical 11-character video ID format.
 * @param {string} videoId Candidate video ID.
 * @return {boolean} True when format is valid.
 */
...
/**
 * Extracts a YouTube video ID from a URL or direct ID input.
 * @param {string} value YouTube URL or raw video ID.
 * @return {string | null} Normalized video ID or null.
 */
  ...
  try {
    ...
    if (host.includes("youtube.com")) {
      ...
      if (pathParts.length >= 2) {
        ...
        const idCandidate = pathParts[1];
        if (["embed", "shorts", "live"].includes(route) &&
          isValidYouTubeVideoId(idCandidate)) {
          return idCandidate;
        }
      }
    }
 ...
 * admin UID.
 */
export const getMasjidAdminCapability = onRequest(
  {
  ...
  async (req, res) => {
    ...
    res.status(200).json({isAdmin: isMasjidAdmin(uid)});
  }
);

/**
 * Admin-only endpoint to create or update a masjid document.
 */
export const adminUpsertMasjid = onRequest(
  {
    secrets: [masjidAdminUids],
    timeoutSeconds: 60,
  },
  async (req, res) => {
    ...
    const masjidRef = masjidId ?
      db.collection("masjids").doc(masjidId) :
      db.collection("masjids").doc();
    const existing = await masjidRef.get();
    ...
    if (state) {
      ...
      data.state = admin.firestore.FieldValue.delete();
    }
    if (imageUrl) {
      data.imageUrl = imageUrl;
    } else {
      data.imageUrl = admin.firestore.FieldValue.delete();
    }
...
/**
 * Admin-only endpoint to queue a YouTube khutbah for processing.
 */
export const adminQueueMasjidKhutbah = onRequest(
  {
    secrets: [masjidAdminUids],
    timeoutSeconds: 60,
  },
  async (req, res) => {
    ...
    const rawMasjidId = req.body?.masjidId;
    const rawYouTubeUrl = req.body?.youtubeUrl;
    const rawTitle = req.body?.title;
    const rawSpeaker = req.body?.speaker;
    const rawManualTranscript = req.body?.manualTranscript;
    const rawDate = req.body?.date;
    const rawDurationSec = req.body?.durationSec;
    const rawTags = req.body?.tags;

    const masjidId = typeof rawMasjidId === "string" ? rawMasjidId.trim() : "";
    const youtubeUrl = typeof rawYouTubeUrl === "string" ?
      rawYouTubeUrl.trim() :
      "";
